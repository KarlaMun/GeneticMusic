...
def objective_function(vector):
    ...
    k = 0.6224*vector[0]*vector[2]*vector[3] + 1.7781*vector[1]*vector[2]*vector[2] + 3.1661*vector[0]*vector[0]*vector[3]+ 19.84*vector[0]*vector[0]*vector[3]
    if -vector[0]+0.0193*vector[2] > 0:
        return 1e18
    if -vector[1]+0.00954*vector[2] > 0:
        return 1e18
    if -pi*vector[2]*vector[2]*vector[3]*vector[3] - 4*pi*vector[2]*vector[2]*vector[2]/3 + 1296000 > 0:
        return 1e18
    if vector[3] - 240 > 0:
        return 1e18
    if vector[0] < 0.0625 or vector[1] < 0.0625 or vector[0] > 0.0625*99 or vector[1] > 0.0625*99 or vector[2] < 10 or  vector[3] > 200 or vector[2] < 0 or  vector[3] < 0:
        return 1e18
    return k
...
search_space = np.array([[-200, +200] for i in range(problem_size)],float) # domains
# algorithm configuration
vel_space = np.array([[-40, +40] for i in range(problem_size)],float)
